...
def loadDataSet():
    xls_file=pd.read_excel('AS5supermarket.xlsx')
    data = np.array(xls_file)
    np.delete(data, [216], axis=1)
    product = []
    for i in range(data.shape[0]):
        product.append(np.where(data[i,]==1)[0])
    return product
...
def aprioriGenerate(Lk, k): #creates Ck
    retList = []
    lenLk = len(Lk)
